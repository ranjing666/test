# 第 2 阶段：成长进阶

- 学习日范围：`Day 011-030`
- 单元范围：`unit_011` 到 `unit_030`

## 这一阶段学什么

- 列表、字典、字符串、文件、调试、模块、类、标准库
- Git / GitHub
- HTML、CSS、JavaScript、DOM
- Flask 路由入口

## 这一阶段怎么学

固定按这个顺序：

1. `01_daily_guides/day_011.md`
2. `02_units/unit_011.md`
3. `03_workbooks/unit_011_workbook.md`
4. `04_code_templates/unit_011_template.md`
5. `05_code_solutions/unit_011_solution.md`
6. `06_quizzes/unit_011_quiz.md`
7. `study_logs/day011.md`
8. `学习进度总看板.md`
9. `编程英语同步学习/100天编程英语路线.md` 对应的 Day 011

## 这一阶段的重点

- 先把 Python 基础能力练到能支撑小项目
- 再进入 Git、前端基础和 Flask 入口
- 先把最小版本做通，再慢慢好看、好用

## 资料入口

- 学习日目录：`01_daily_guides/README.md`
- 单元讲义目录：`02_units/README.md`
- 工作簿目录：`03_workbooks/README.md`
- 起步模板目录：`04_code_templates/README.md`
- 参考答案目录：`05_code_solutions/README.md`
- 小测目录：`06_quizzes/README.md`
- 项目包目录：`07_project_packs/README.md`
- 配套英语目录：`../编程英语同步学习/README.md`
- 总导航：`../学习顺序总导航.md`

## 新增详细教材入口

这一阶段现在已经补上了按单元展开的详细讲义。
本阶段对应单元范围：`unit_011` 到 `unit_030`。
详细目录在：`02_units/README.md`



